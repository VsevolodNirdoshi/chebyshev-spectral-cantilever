...
def plot_second_kind_comparison(prefix="compare2_", folder=".", show_errors=True):
    ...
    ax.plot(xL, yL, linewidth=LW_INTERP, label="Second kind Lobatto interpolant u_L(x)")
    ax.plot(xG, yG, linewidth=LW_INTERP, label="Second kind Gauss interpolant u_G(x)")

    if xNL.size:
        ax.scatter(xNL, yNL, s=NODE_SIZE, label="Second kind Lobatto nodes")
    if xNG.size:
        ax.scatter(xNG, yNG, s=NODE_SIZE, label="Second kind Gauss nodes")

    if show_errors and rL.size and rG.size:
        axr.plot(xrL, rL, linewidth=LW_RESIDUAL, linestyle="--",
                 label="Second kind residual r_L(x)")
        axr.plot(xrG, rG, linewidth=LW_RESIDUAL, linestyle="--",
                 label="Second kind residual r_G(x)")
    ...
    ax.set_title("Second-kind comparison: f(x), interpolants, nodes, and residuals")
    ax.set_xlabel("x")
    ax.set_ylabel("value")
    ...
    # ---- table ----
    rows = [
        [r"Second kind Lobatto max $|r_L(x)|$", sci_mathtext(eLmax, digits=3) if eLmax > 0 else r"$\mathrm{n/a}$"],
        [r"Second kind Gauss max $|r_G(x)|$",   sci_mathtext(eGmax, digits=3) if eGmax > 0 else r"$\mathrm{n/a}$"],
...
if __name__ == "__main__":
    plot_second_kind_comparison(prefix="compare2_", folder=SCRIPT_DIR, show_errors=True)
